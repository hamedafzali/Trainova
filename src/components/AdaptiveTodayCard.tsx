...
export function AdaptiveTodayCard({ onStart }: { onStart: (templateId: string) => void }) {
  ...
  return (
    <section className={`card border-l-4 ${recommendation.mode === "recovery" ? "border-amber" : "border-accent"}`}>
      <p className="section-label">Adaptive Today</p>
      <h2 className="mt-1 text-xl font-bold text-ink">{recommendation.headline}</h2>
      <p className="mt-2 text-base font-semibold text-ink">{recommendation.planName}</p>
      <p className="mt-1 text-sm leading-6 text-inkSoft">{recommendation.reason}</p>
